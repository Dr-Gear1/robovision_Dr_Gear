...
def to_rgb(image: np.ndarray, src: str = "bgr") -> np.ndarray:
    """
    Convert an image to a 3-channel RGB array.

    Mirrors the behaviour of OpenCV's ``cv2.cvtColor``:

    * **BGR input** (``src='bgr'``, default) — swaps the B and R channels
      so the image displays correctly with Matplotlib / RGB-first libraries.
      Equivalent to ``cv2.cvtColor(img, cv2.COLOR_BGR2RGB)``.
    * **Grayscale input** (2-D array) — replicates the single channel into
    ...
    image : np.ndarray
        ...
        - ``(H, W)``   – grayscale; channels are replicated → (H, W, 3).
        - ``(H, W, 1)``– single-channel; squeezed then replicated → (H, W, 3).
        - ``(H, W, 3)``– 3-channel; B and R are swapped when ``src='bgr'``.
        - ``(H, W, 4)``– RGBA/BGRA; alpha is dropped, then B↔R swap applied.

        Dtype may be float32 / float64 (range [0, 1]) or uint8 ([0, 255]).
    src : str, optional
        Declare the channel order of a 3- or 4-channel *image*:

        - ``'bgr'`` (default) – input is BGR (OpenCV convention); B and R
          channels are swapped to produce RGB.
        - ``'rgb'`` – input is already RGB; returned unchanged.

        Ignored for grayscale (2-D or single-channel) inputs.
    ...
    TypeError
        If *image* is not a NumPy ndarray.
    ValueError
        If *image* has an incompatible shape, or *src* is not ``'bgr'``
        or ``'rgb'``.

    Examples
    --------
    >>> # OpenCV-loaded image (BGR) → RGB for Matplotlib display
    >>> bgr = cv2.imread("photo.jpg")               # (H, W, 3) BGR
    >>> rgb = to_rgb(bgr)                           # (H, W, 3) RGB  ✔

    >>> # Grayscale → 3-channel (achromatic)
    >>> gray = read_image("photo.png", as_gray=True)
    >>> rgb  = to_rgb(gray)                         # (H, W, 3) R=G=B
    """
    _validate_ndarray(image, "image")
    _validate_image_shape(image, "image")

    if src not in ("bgr", "rgb"):
        raise ValueError(f"'src' must be 'bgr' or 'rgb', got {src!r}.")

    # ── Grayscale inputs: replicate channel (identical to cv2.COLOR_GRAY2RGB) ─
    if image.ndim == 2:
    ...
    # ── Drop alpha if present (RGBA / BGRA → RGB / BGR) ──────────────────────
    if image.ndim == 3 and image.shape[2] == 4:
        image = image[:, :, :3]

    # image is now (H, W, 3) ──────────────────────────────────────────────────
    if src == "rgb":
        return image                            # already RGB, nothing to do

    # src == 'bgr': swap channel 0 (B) with channel 2 (R)  ←→  COLOR_BGR2RGB
    return image[:, :, ::-1].copy()
